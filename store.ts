...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  handleCreateInstallmentPlan: async (cardId, principal, annualRate, totalPeriods, startDate, notes) => {
    ...
    const newCards = cards.map(c => {
      if (c.id !== cardId) return c;
      const { lastBillDateObj } = getStatementRange(c.billDay);
      let { currentUnpaid, currentUnbilled } = c;
      // Installment start principal counts as new debt in the current billing cycle
      if (new Date(startDate) > lastBillDateObj) {
        currentUnbilled += principal;
      } else {
        currentUnpaid += principal;
      }
      return { ...c, currentUnpaid, currentUnbilled, transactions: [...txs, ...c.transactions] };
    });
