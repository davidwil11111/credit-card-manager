...
function makeDefaultForm(): Partial<CreditCard> {
  return {
    holderName: '',
    bankName: '',
    cardNumber: '',
...
export const CreditCardForm: React.FC<CreditCardFormProps> = ({ initialData, onSubmit, onCancel }) => {
  const [formData, setFormData] = useState<Partial<CreditCard>>(() =>
    initialData ? { ...initialData } : makeDefaultForm()
  );
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 rounded-lg border bg-gray-50 ${errors.bankName ? 'border-red-500' : 'border-gray-200'}`}
                placeholder="选择或输入"
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-2 rounded-lg border bg-gray-50 font-mono ${errors.cardNumber ? 'border-red-500' : 'border-gray-200'}`}
                placeholder="8888"
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleChange('billDay', parseInt(e.target.value))}
              ...
              <div>
                ...
                <input
                  type="number"
                  step="0.01"
                  value={formData.fixedLimit || formData.fixedLimit === 0 ? formData.fixedLimit : ''}
                  onChange={(e) => handleChange('fixedLimit', parseFloat(e.target.value) || 0)}
                  className="w-full p-2 rounded-lg border border-gray-200 bg-gray-50"
                  placeholder="0.00"
                />
              </div>
            ...
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
              ...
              <div className="flex flex-col gap-3">
                <label className="flex items-center gap-2">
                  <input
                    type="radio"
                    checked={formData.repaymentConfig?.type === 'fixed_day'}
                    onChange={() => handleRepaymentConfigChange('fixed_day', 10)}
                    className="text-blue-600"
                  />
                  <span className="text-sm">每月固定</span>
                  {formData.repaymentConfig?.type === 'fixed_day' && (
                    <select
                      value={formData.repaymentConfig.value}
                      onChange={(e) => handleRepaymentConfigChange('fixed_day', parseInt(e.target.value))}
                      className="ml-auto text-sm p-1 border rounded"
                    >
                ...
                <label className="flex items-center gap-2">
                  <input
                    type="radio"
                    checked={formData.repaymentConfig?.type === 'days_after_bill'}
                    onChange={() => handleRepaymentConfigChange('days_after_bill', 20)}
                    className="text-blue-600"
                  />
                  ...
                  {formData.repaymentConfig?.type === 'days_after_bill' && (
                    <div className="ml-auto flex items-center gap-1">
                      <input
                        type="number"
                        value={formData.repaymentConfig.value}
                        onChange={(e) => handleRepaymentConfigChange('days_after_bill', parseInt(e.target.value))}
