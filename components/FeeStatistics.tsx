...
import { ArrowLeft, Download, TrendingUp, CreditCard, DollarSign, Calendar, ChevronDown } from 'lucide-react';
...
export const FeeStatistics: React.FC<FeeStatisticsProps> = ({ onBack }) => {
  ...
  const selectedLabel = useMemo(() => {
    if (selectedCardIds.length === 0 || selectedCardIds.length === cards.length) return '全部卡片';
    if (selectedCardIds.length === 1) {
      const c = cards.find(x => x.id === selectedCardIds[0]);
      return c ? `${c.bankName} · ${c.holderName}` : '全部卡片';
    }
    return `已选 ${selectedCardIds.length} 张`;
    ...
    <div className="flex flex-col h-full bg-[#f4f5f9]">
      ...
      <div className="bg-white pt-12 pb-2 px-4 sticky top-0 z-30 shadow-sm">
        ...
        <div className="mt-2 relative" ref={dropdownRef}>
          ...
          >
            <span className={selectedCardIds.length === 0 ? 'text-gray-800' : 'text-blue-600 font-medium'}>{selectedLabel}</span>
          ...
          {dropdownOpen && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-gray-200 z-50 max-h-60 overflow-y-auto py-1">
              <label className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 cursor-pointer border-b border-gray-50">
                <input
                  type="checkbox"
                  checked={selectedCardIds.length === 0}
