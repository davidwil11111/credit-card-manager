        ...
        <section className="space-y-3">
          ...
          {reminders.paidCount > 0 && (
            <div className="rounded-2xl p-4 flex items-center justify-between border border-emerald-100 shadow-sm bg-emerald-50/50">
              ...
              <button className="bg-emerald-600 text-white px-4 py-1.5 rounded-xl text-xs font-semibold hover:bg-emerald-700 transition-colors shadow-sm">去查看</button>
            </div>
          )}
        </section>
          ...
          {sortedCards.map(card => {
            ...
            return (
              <div key={card.id} onClick={e=>isSelectionMode?toggleSelection(card.id,e):onSelectCard(card)}
                ...
                <div className="text-right"><div className={'text-[10px] font-bold px-2 py-0.5 rounded-full inline-block '+statusBg}>{isPaid?'已还清':card.status==='overdue'?'逾期':'待还款'}</div><div className="text-xs mt-1"><span className="font-bold" style={{color:statusColor}}>{formatRepaymentDate(card.repaymentDate)}</span><span className="text-slate-400"> ({days>0?days+'天后':'已逾期'})</span></div></div></div>
