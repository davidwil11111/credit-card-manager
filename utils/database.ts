...
class Database {
  ...
  private async migrateIfNeeded(): Promise<void> {
    ...
    try {
      ...
      if (!saColumns.includes('statement_amount')) {
        ...
        logger.info('Migration: added statement_amount column');
      }
    } catch (error) {
  ...
  async saveCards(cards: CreditCard[]): Promise<void> {
    ...
    if (this.isNative) {
      ...
      const insertSQL = `INSERT INTO cards (id, holder_name, bank_name, card_number, bill_day, repayment_config, repayment_date, last_statement_date, fixed_limit, temp_limit, temp_limit_expiry, current_unpaid, current_unbilled, statement_amount, status, transactions, index_col)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
